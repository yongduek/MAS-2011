# https://github.com/opencv/opencv/blob/4.x/samples/python/gabor_threads.py
#!/usr/bin/env python
...
def process(img, filters, return_all=False):
    accum = np.zeros_like(img)
    fimglist = []
    for kern in filters:
        fimg = cv.filter2D(img, cv.CV_8UC3, kern)
        np.maximum(accum, fimg, accum)
        fimglist.append(fimg) 
        
    if return_all:
        return accum, fimglist 
    else:
        return accum
