...
def main():
    ...
    columns = np.zeros((height, nframes*2, 3), dtype='uint8')
    ...
    while video.isOpened():
        ...
        # 1. block shift to the left & right
        columns[:,:half,:] = columns[:,1:half+1,:]
        columns[:, half+2:, :] = columns[:, half+1:-1, :]
        
        # 2. put the new column at the last of the column
        columns[:,half,:] = array[:, array.shape[1]//2, :]
        columns[:,half+1,:] = array[:, array.shape[1]//2 + 1, :]
